...
function brick_rsvp_ajax($form, $form_state) {
  ...
  try {
    ...
    if (brick_event_full($event_node)) {
      ...
      // -- You snooze, you lose!
      drupal_set_message(t('RSVP Failed: The event has become full'), 'error');
    ...
    drupal_set_message(t('There was an error RSVPing for the event. Please try again or contact us if you still are having trouble.'), 'error');
  }
